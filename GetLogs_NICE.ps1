...
if((Test-Path "$ChoosenPath\LogCopied_$($date.day)") -ne $true )
{
...
$MonDossier_Clear = Get-ChildItem -Path $ChoosenPath\LogCopied_$($date.day) -Recurse -file | Where-Object{$_.LastWriteTime.date -eq $ChoosenDate} #On récupère la liste des fichiers de ce répertoire
...
$MonFolder = Get-ChildItem -Path $LogPath -Recurse -file | Where-Object{$_.LastWriteTime.date -eq $ChoosenDate} #On récupère la liste des fichiers de ce répertoire
foreach ($MyFile in $MonFolder)
{
    ...
    Copy-item $Myfile.FullName -Destination $ChoosenPath\LogCopied_$($date.day) -Force
